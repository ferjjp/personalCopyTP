...
void removeFDFromPoll(int fd, struct pollfd** arraySocketsPCP, int* cantSocketsCPUAbiertos) {
	int i = 0;
	while ((i < *cantSocketsCPUAbiertos)
			&& ((*arraySocketsPCP)[i].fd != fd)) {
		i++;
	};
...
void removeCPUFromPoll(t_cpu* cpu, struct pollfd** arraySocketsPCP,
	...
	removeFDFromPoll(cpu->socket,arraySocketsPCP,cantSocketsCPUAbiertos);
}
