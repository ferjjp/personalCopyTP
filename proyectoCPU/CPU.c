...
t_pcb* getNewPCB() {
	...
	*new = *(t_pcb*) recepcion_PCB->data;
	...
	log_debug(logger, "PCB Received correctly. Starting to execute process %d",new->pid);
	return new;
}
...
void traer_segmento_etiquetas() {
	...
	if (currentPCB->tamanioIndiceEtiquetas != 0) {
		aux = solicitar_algo_UMV(currentPCB->indiceEtiquetas, 0,
				currentPCB->tamanioIndiceEtiquetas);
		diccionarioEtiquetas = realloc(diccionarioEtiquetas,currentPCB->tamanioIndiceEtiquetas);
